...
import { SOCIAL_PLATFORM_LABELS, ensureLayoutBlocks, normalizeLayoutV2 } from '@/lib/profileBlocks'
...
function commitContactFields() {
  layout.value = normalizeLayoutV2({
    ...
    blocks: normalized.value.blocks.map((block) => {
      ...
      const hasValue = Boolean(phone.value || zalo.value || messenger.value)
      return {
        ...block,
        active: hasValue,
...
function addSocialLink() {
  socialItems.value = [...socialItems.value, { platform: 'instagram', label: '', url: '' }]
}
...
<template>
  <div class="space-y-5">
    <div class="profile-form-section">
      <p class="profile-form-section__title">Liên hệ trực tiếp</p>
      <p class="mt-1 text-sm text-slate-400">Hiển thị ngay dưới phần hero trên trang công khai.</p>
      <div class="mt-4 grid gap-4 md:grid-cols-3">
        <div>
          <label class="mb-2 block text-sm text-slate-300">Điện thoại</label>
          <input v-model="phone" class="field" type="text" placeholder="0901234567" @input="commitContactFields" />
        </div>
        <div>
          <label class="mb-2 block text-sm text-slate-300">Zalo</label>
          <input v-model="zalo" class="field" type="text" placeholder="Zalo ID / SĐT" @input="commitContactFields" />
        </div>
        <div>
          <label class="mb-2 block text-sm text-slate-300">Messenger</label>
          <input v-model="messenger" class="field" type="text" placeholder="Facebook / Messenger" @input="commitContactFields" />
        </div>
      </div>
    </div>
    ...
    <div class="profile-form-section">
      <div class="flex flex-wrap items-start justify-between gap-3">
        ...
        <button class="btn-secondary text-sm" type="button" @click="addSocialLink">+ Thêm link</button>
      </div>
      ...
      <div v-for="(item, index) in socialItems" :key="index" class="block-editor-item mt-4">
        <div class="grid gap-3 md:grid-cols-2">
          <div>
            ...
            >
              <option v-for="(label, platform) in SOCIAL_PLATFORM_LABELS" :key="platform" :value="platform">
                {{ label }}
              </option>
            </select>
          </div>
        ...
        <div class="mt-3 flex justify-end">
          <button class="btn-secondary px-3 py-2 text-xs text-rose-200" type="button" @click="removeSocial(index)">
            Xóa
          </button>
        </div>
      </div>
